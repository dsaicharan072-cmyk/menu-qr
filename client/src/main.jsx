...
function Admin() { const [restaurants,setRestaurants]=useState([]); const [selected,setSelected]=useState(null); const [tab,setTab]=useState('overview'); const token=localStorage.getItem('menuqr-token'); const nav=useNavigate(); useEffect(()=>{if(!token) nav('/admin/login'); else api('/restaurants').then(setRestaurants).catch(()=>nav('/admin/login'));},[token,nav]); const current=selected||restaurants[0]; const url=current?`${location.origin}${location.pathname}#/r/${current.slug}`:''; return <div className="admin"><aside><div className="logo"><UtensilsCrossed/> MenuQR</div><button className={tab==='overview'?'selected':''} onClick={()=>setTab('overview')}>Overview</button><button className={tab==='restaurants'?'selected':''} onClick={()=>setTab('restaurants')}>Restaurants</button><button className={tab==='qr'?'selected':''} onClick={()=>setTab('qr')}><QrCode size={17}/> Menu QR</button><button className="logout" onClick={()=>{localStorage.removeItem('menuqr-token');nav('/admin/login')}}><LogOut size={17}/> Sign out</button></aside><section className="dashboard"><header><div><p className="eyebrow">Restaurant administration</p><h1>{tab==='qr'?'Menu QR code':tab==='restaurants'?'Restaurants':'Overview'}</h1></div><Link className="open-menu" to={current?`/r/${current.slug}`:'#'} target="_blank">Open guest menu</Link></header>{tab==='overview'&&<><div className="stats"><div><small>Restaurants</small><b>{restaurants.length}</b></div><div><small>Live menu items</small><b>{restaurants.reduce((n,r)=>n+r.itemCount,0)}</b></div><div><small>Menu QR codes</small><b>{restaurants.length}</b></div></div><div className="panel"><h2>Your restaurants</h2>{restaurants.map(r=><button key={r.id} className="restaurant-row" onClick={()=>setSelected(r)}><span><b>{r.name}</b><small>{r.address}</small></span><span>{r.itemCount} dishes</span></button>)}</div></>}{tab==='restaurants'&&<div className="panel"><h2>Restaurant directory</h2><p>Each restaurant has one public menu URL and one QR code.</p>{restaurants.map(r=><div className="restaurant-row" key={r.id}><span><b>{r.name}</b><small>{r.slug} · {r.phone}</small></span><Link to={`/r/${r.slug}`} target="_blank">Preview</Link></div>)}</div>}{tab==='qr'&&current&&<div className="qr-layout"><div className="panel"><h2>{current.name}</h2><p>This single QR code opens the complete menu. Print it once and use it anywhere in the restaurant.</p><p><b>Menu link:</b> {url}</p></div><div className="qr-card"><QRCodeSVG value={url} size={220}/><h3>{current.name}</h3><p>Restaurant menu</p><small>{url}</small></div></div>}</section></div> }
function App(){return <Routes><Route path="/" element={<Scanner/>}/><Route path="/scan" element={<Scanner/>}/><Route path="/demo-qr" element={<DemoQr/>}/><Route path="/r/:restaurantId" element={<CustomerMenu/>}/><Route path="/menu/restaurant/:restaurantId" element={<CustomerMenu/>}/><Route path="/admin/login" element={<AdminLogin/>}/><Route path="/admin/*" element={<Admin/>}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes>};
